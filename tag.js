...
for(var w = 0; w < numBits; w++){
     xPos.push(round(random(50, 350)));
     yPos.push(round(random(50, 350)));
     bitSpeed.push(random(1, 2));
    }
    
var draw = function() {
     
    background(131, 161, 242);

    for(var q = 0; q < xPos.length; q++){
    
        drawBitmoji(xPos[q], yPos[q], 50);
        xPos[q] += bitSpeed[q];
         if(xPos[q] >= 400){
            xPos[q] = 0;
    }
...
mouseClicked = function(){
    ...
    for(var e = 0; e < xPos.length; e++){
        ellipse(mouseX, mouseY, 50, 50);
        if(mouseX > xPos[e]-21 && mouseX < xPos[e]+21 && mouseY > yPos[e]-21 && mouseY < yPos[e]+21){
                        xPos[e] = -10;
        }
        }
        };
